...
public class IshtMDBDao_shyam extends MongoBaseDao {
    ...
    public ResultObject getIshtJSONObject(RootMDB root){
    	...
    	List<IshtLineMDB> finlLine =  new ArrayList <IshtLineMDB>(); //Creating new list add RitwikName
    	
    	List<FamilyMDB> familyList = root.getFamily();
    	//logger.info("family list count" + familyList.size());
    	IshtMDB isht = new IshtMDB();
    	IshtMDB Finalisht = new IshtMDB();
    	try {
    		...
    		if (list!=null && !list.isEmpty() ) { //||   (test != null && !test.isEmpty()
    			logger.info("Size of list after if  : "+list.size());
    			...
    			while(ishtLine.hasNext()){  
    				...
    				list1.add(ishtLineObj.getId());           //adding only ID to the main list. 
    				
    				//iterate family object to ritwik Name
    				if(familyList!=null && !familyList.isEmpty()){
        				...
        				while(itr.hasNext()){  
        					...
        					if(ishtLineObj.getId().equalsIgnoreCase(family.getPersonalID())) {
        						...
        						finlLine.add(ishtLineObj);	
        					}
        				}
        				list2.add(root.getPersonalId());  //adding personal ID to the main list.
        			}
    				//iterate family object to ritwik Name
    			...
    			logger.info("Before Removal"+ list2.size());

    			logger.info("getting remaning objects of family "+ list2);
    			list2.removeAll(list1); //removing from the list2 all the object list1
    			...
    			for(int i=0; i<=list2.size()-1;i++ ){
    				...
    				while(itr.hasNext()){  
    					...
    					if(family.getPersonalID().equalsIgnoreCase(list2.get(i).toString())){
    						...
    						logger.info("Name :  "+ family.getFirstName().toUpperCase()+ " "+family.getLastName().toUpperCase());
    						istLine.setId(family.getPersonalID());
    						istLine.setSwastyayani(istLine.getSwastyayani()); //0.00d
    						istLine.setIstavrity(istLine.getIstavrity());
    						istLine.setAcharyavrity(istLine.getAcharyavrity());
    						istLine.setDakshina(istLine.getDakshina());
    						istLine.setSangathani(istLine.getSangathani());
    						istLine.setPronami(istLine.getPronami());
    						istLine.setSurplus(istLine.getSurplus());
    						istLine.setRitwiki(istLine.getRitwiki());
    						istLine.setTotal(istLine.getTotal());
    						istLine.setRitwik(family.getrName());
    						logger.info("Ritwik Name :  "+ family.getrName());
    						list.add(istLine);
    					}
    				}
    			}
    			isht.setLine(list);
    			Finalisht.setLine(finlLine);
    		}  // end of List Loop 
    		else
    		{
    		...
    		//resultObject.setObject1(Finalisht);
